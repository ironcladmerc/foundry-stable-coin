 ...
 * collateral be <= to the $ backed value of the DSC
...
contract DSCEngine is ReentrancyGuard {
    ...
    uint256 private constant LIQUITDATION_THRESHOLD = 50; // 200% collateralized
     ...
     * @param amountDscToMint the amount of DSC to mint
     */
    // Check if the collateral amount is > DSC amount
    function mintDsc(
        uint256 amountDscToMint
    ...
    function burnDsc(
        ...
        _revertIfHealthFactorIsBroken(msg.sender); // this would probably never happen
    }
     ...
     * their health factor should be below min health factor
     * @notice you can partially liquidate a user
     * @notice you will get a liquidation bonus for taking a user's funds
     * @notice this function working assumes the protocol will be roughly 200% overcollateralized in order for this to work
     * @notice a known bug would be if the protocol were 100% or less collateralized, we wouldn't be able to incentivize liquidators
    ...
    function _healthFactor(address user) private view returns (uint256) {
        ...
        // return the value of all collateral / total DSC minted
        (
            uint256 totalDscMinted,
            uint256 collateralValueInUsd
        ) = _getAccountInformation(user);
        uint256 collateralAdjustedForThreshold = (collateralValueInUsd *
            LIQUITDATION_THRESHOLD) / LIQUIDATION_PRECISION;
    ...
    function getUsdValue(
        ...
        return
            (uint256(price) * amount * ADDITIONAL_FEED_PRECISION) / PRECISION;
    }
